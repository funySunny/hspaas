package com.huashi.exchanger.resolver.sms.http.cmccheli;
...
public class CmccheliPassageResolver extends AbstractPassageResolver {
    ...
    public List<ProviderSendResponse> send(SmsPassageParameter parameter, String mobile, String content,
        ...
        try {
            ...
            if (!isMoPushRunning) {
                try {
                    startMoPush(headers, tparameter.getString("mo_subscribe_url"),
                                tparameter.getString("mo_callback_url"), tparameter.getString("terminal_no"),
            ...
            // 转换参数，并调用网关接口，接收返回结果
            String result = HttpClientManager.postJson(parameter.getUrl(),
                                                       headers,
                                                       sendMtRequestParameter(tparameter,
                                                                              mobile,
                                                                              content,
                                                                              extNumber,
                                                                              parameter.getSmsTemplateId(),
                                                                              buildVariableParamsReport(parameter.getVariableParamNames(),
                                                                                                        parameter.getVariableParamValues())));
    ...
    private static String pickupSignature(String content) {
        ...
        return content.substring(content.lastIndexOf(MESSAGE_SIGNATURE_PRIFIX) + 1,
                                 content.lastIndexOf(MESSAGE_SIGNATURE_SUFFIX));
    }
     ...
     * @return
     */
    private static String sendMtRequestParameter(TParameter tparameter, String mobile, String content,
        ...
        if (StringUtils.isNotBlank(terminalNo)) params.put("smsCustomCode",
                                                           terminalNo
                                                                   + (StringUtils.isNotBlank(extNumber) ? extNumber : ""));
    ...
    private static List<ProviderSendResponse> sendResponse(String result, String successCode) {
        ...
        // 手机号码报文信息， eg: "msgList": [{"mobile": "18867103703","resultCode": 416,"resultMsg":
        // "mobile belong to black list"}]
        List<Map<String, Object>> mobileReport = JSON.parseObject(report.getString("msgList"),
     ...
     * @return
     */
    private void startMoPush(Map<String, Object> headers, String moUrl, String moCallbackUrl, String terminalNo,
                             String successCode) {
        try {
