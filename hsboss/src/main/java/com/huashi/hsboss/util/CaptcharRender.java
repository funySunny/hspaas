 ...
 * 验证码生成类
 * @author yangm
 * @create_at 2015年4月21日 下午6:03:39
 */
public class CaptcharRender extends Render {
	...
	public static String outputVerifyImage(int w, int h, File outputFile,
		...
		drawGraphic(w, h, outputFile, verifyCode);
		return verifyCode;
	}
	 ...
	 * @throws IOException
	 */
	public String outputVerifyImage(OutputStream os, int verifySize)
			throws IOException {
		String verifyCode = generateVerifyCode(verifySize);
		drawGraphic(imgWidth, imgHeight, os, verifyCode);
		return verifyCode;
	}
	 ...
	 * @throws IOException
	 */
	public static void drawGraphic(int w, int h, File outputFile, String code)
			throws IOException {
		if (outputFile == null) {
			return;
		}
	 ...
	 * @throws IOException
	 */
	public static void drawGraphic(int w, int h, OutputStream os, String code)
			throws IOException {
	...
	public CaptcharRender() {
		this(6);
	}

	public CaptcharRender(int imgRandNumber) {
		if (imgRandNumber < 4) {
			imgRandNumber = 4;
		}
		randomCode = generateVerifyCode(imgRandNumber);
	}

	public CaptcharRender(int imgRandNumber, int width, int height) {
		if (imgRandNumber < 4) {
			imgRandNumber = 4;
		}
		imgWidth = width;
	...
	public void render() {
		...
		try {
			...
			drawGraphic(imgWidth, imgHeight, sos, randomCode);
		} catch (Exception e) {
