...
public class MobileNumberCatagoryUtil {
	...
	public static MobileCatagory doCatagory(List<String> mobiles) {
		...
		try {
			...
			logger.error("{} 号码分流错误, 信息为: {}", StringUtils.join(mobiles, DATA_SPLIT_CHARCATOR) , e.getMessage());
			response.setSuccess(false);
			response.setMsg("号码分流错误");
		}
		logger.info(response.toString());
	...
	public static class MobileCatagory implements Serializable {
		...
		public String toString() {
			return "MobileCatagory [处理结果=" + success + ", 移动号码数=" + cmSize
					+ ", 联通号码数=" + cuSize + ", 电信号码数=" + ctSize
					+ ", 过滤号码数=" + filterSize + ", 过滤信息="
					+ filterNumbers + "]";
		}
