...
public class MmsSendbackController extends BaseController {
     ...
     * TODO 接收通道厂商回执状态报告
     *
     * @param filterCode 过滤码
     * @param providerCode 通道简码
     * @param encoding 编码方式
     * @return
     */
    ...
    public String status(@PathVariable("filter_code") Integer filterCode,
                         @PathVariable("provider_code") String providerCode, @PathVariable("encoding") String encoding) {
        try {
    ...
    @ResponseBody
    public String mo(@PathVariable("filter_code") Integer filterCode,
                     @PathVariable("provider_code") String providerCode, @PathVariable("encoding") String encoding) {
        try {
     ...
     * @return
     */
    @RequestMapping("/model/{filter_code}/{provider_code}/{encoding}")
    @ResponseBody
    public String model(@PathVariable("filter_code") Integer filterCode,
                        @PathVariable("provider_code") String providerCode, @PathVariable("encoding") String encoding) {
        try {
            mmsPassagePrervice.doPassageMoReport(providerCode, doTranslateParameter(filterCode, encoding));
        } catch (Exception e) {
